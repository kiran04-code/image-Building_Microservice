...
export async function buildRepository(repoUrl: string, branch?: string): Promise<BuildResult> {
...
function statusOf(build: Build): BuildStatus {
    if (build.buildComplete) {
        if (build.buildStatus === "SUCCEEDED") return "success";
...
export async function getBuild(id: string): Promise<BuildResult | undefined> {
