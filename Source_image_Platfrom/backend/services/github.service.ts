...
async function githubFetch(path: string, useAuth: boolean = true) {
  ...
  if (response.status === 401) throw new Error("INVALID_GITHUB_TOKEN");
...
async function readTextFile(owner: string, repo: string, path: string, branch: string) {
  let response: globalThis.Response;
  try {
...
function hasProjectMarkers(files: Set<string>): boolean {
  return (
...
export async function inspectGitHubRepository(owner: string, repo: string, requestedBranch?: string): Promise<GitHubRepositoryFiles> {
  const metadata = (await (await githubFetch(`/repos/${owner}/${repo}`)).json()) as { default_branch?: string };
  const defaultBranch = requestedBranch || metadata.default_branch || "main";
  const root = (await (await githubFetch(`/repos/${owner}/${repo}/contents?ref=${encodeURIComponent(defaultBranch)}`)).json()) as Array<{ name: string; type?: string }>;
  ...
  if (!hasProjectMarkers(files)) {
    ...
    for (const dir of subDirs) {
      try {
        const subContents = (await (await githubFetch(`/repos/${owner}/${repo}/contents/${encodeURIComponent(dir)}?ref=${encodeURIComponent(defaultBranch)}`)).json()) as Array<{ name: string }>;
  ...
  if (packageText) {
    try { packageJson = JSON.parse(packageText) as Record<string, unknown>; }
    catch { throw new Error("INVALID_PACKAGE_JSON"); }
  }
